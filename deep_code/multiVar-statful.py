...
var_num = dataset.shape[1]-1
# 입력을 (샘플 수, 타임스텝, 특성 수)로 형태 변환
x_train = np.reshape(x_train, (y_train.shape[0], window_size, var_num))
first_layer_node_cnt = var_num*(var_num-1)/2

# 3. 모델 구성하기
model = Sequential()
model.add(LSTM(first_layer_node_cnt, batch_input_shape=(var_num, window_size, var_num), stateful=True))
