...
def seq2dataset(seq, window_size):
    dataset_X = []
    dataset_Y = []
    print(range(len(seq) - window_size))
    for i in range(len(seq) - window_size): # range(54-4=50) 0~50 . 51회 루프.
        subset = seq[i:(i + window_size + 1)]  # 0~4=5 (앞으로 얼마를 계산하냐에 따른 것인듯.
        for si in range(len(subset) - 1):  # 4회. 루프. 나머지 1은 Y로  4가 윈도우 사이즈 이고 3대 1로 X-Y을 고려해서인가?
            # print("subset")
            # print(subset[si])
            features = code2features(subset[si]) # 2글자 글을 나누는 식.
            dataset_X.append(features)

        dataset_Y.append([code2idx[subset[window_size]]]) # subset의 5번째. 번호는 4. Y는 여기에.
...
seq = ['g8', 'e8', 'e4', 'f8', 'd8', 'd4', 'c8', 'd8', 'e8', 'f8', 'g8', 'g8', 'g4',
       ...
       'g8', 'e8', 'e4', 'f8', 'd8', 'd4', 'c8', 'e8', 'g8', 'g8', 'e8', 'e8', 'e4']  # 54개
