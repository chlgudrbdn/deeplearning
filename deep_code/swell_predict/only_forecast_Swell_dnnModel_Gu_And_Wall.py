...
# history = model.fit(X, Y, validation_split=0.2, epochs=epochs, verbose=2, callbacks=[early_stopping_callback])
history = model.fit(X, Y, validation_split=0.2, epochs=10, verbose=2, callbacks=[early_stopping_callback, checkpointer])
...
prediction_for_test = np.where(model.predict(X_test) <= 0.5, 0, 1)
