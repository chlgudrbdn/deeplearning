...
def search_best_model(MODEL_DIR):
    ...
    print(file_list[-1])
    model = load_model(MODEL_DIR + '{0:.9f}'.format(file_list[-1]) + ".hdf5")
    return model
...
for train_index, validation_index in kf.split(X_train_and_validation):  # 이하 모델을 학습한 뒤 테스트.
    ...
    fig.canvas.set_window_title(scriptName + ' model_loopNum' + str(len(lossList)).zfill(2))
    ax2 = ax1.twinx()
    ax1.plot(x_len, y_acc, c="blue", label='acc')
    ax1.plot(x_len, y_vacc, c="red", label='val_acc')
    ax2.plot(x_len, y_loss, c="green", label='loss')
    ax2.plot(x_len, y_vloss, c="orange", label='val_loss')
