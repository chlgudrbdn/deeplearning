...
for num in range(0, len(test_only_date), 3):  # 50회 루프가 있을 것이다.
    ...
    for i in range(epochs):
        model.fit(X_train, y_train, epochs=1, batch_size=n_batch, verbose=0, shuffle=False, validation_data=(X_val, y_val),
...
# 다루는 데이터 범위에서 텍스트 파트를 줄여도 2epoch 10번 루프에 29분. 50번 루프에는 약 145분. 50epoch라면 7250분. 120시간 소요 예상. 100 epoch라면 240시간. 제출일자 초과.
"""
# -*- coding: utf-8 -*-
...
for num in range(0, len(test_only_date), 3):  # 50회 루프가 있을 것이다.
    ...
    model = Sequential()
    model.add(LSTM(number_of_var, batch_input_shape=(number_of_var, look_back, number_of_var),
                   stateful=True, return_sequences=True))
    model.add(Dropout(0.1))
    model.add(LSTM(number_of_var, batch_input_shape=(number_of_var, look_back, number_of_var), stateful=True, return_sequences=True))
    model.add(Dropout(0.1))
    # model.add(LSTM(first_layer_node_cnt/9, batch_input_shape=(number_of_var, look_back, number_of_var), stateful=True, return_sequences=True))
    # model.add(Dropout(0.1))
    model.add(Dense(1))
    model.compile(loss='mean_squared_error', optimizer='adam')
    ...
    for i in range(epochs):
        model.fit(X_val, y_val, epochs=1, batch_size=n_batch, verbose=0, shuffle=False, validation_data=(X_train, y_train),
                  callbacks=[custom_hist, checkpointer])
        model.reset_states()
